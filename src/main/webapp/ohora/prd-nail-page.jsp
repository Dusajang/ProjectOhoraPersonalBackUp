		...
		<div id="second-wrap-layout">
			...
			<div id="item-list-wrap">
				...
				<div id="page-container">    
				    <a href="?catno=${categoryNumber}&currentPage=1&sort=new<c:if test='${param.searchWord != null}'>&searchWord=${param.searchWord}</c:if>" class="first">first</a>            
				    
				    <c:if test="${pvo.currentPage > 1}">
				        <a href="?catno=${categoryNumber}&currentPage=${pvo.currentPage - 1}&sort=new<c:if test='${param.searchWord != null}'>&searchWord=${param.searchWord}</c:if>" class="prev">prev</a>
				    </c:if>                
				    ...
				    <ol>
				        <c:forEach begin="${pvo.start}" end="${pvo.end}" step="1" var="i">
				            <c:choose>                
				                ...
				                <c:otherwise>
				                    <li><a href="?catno=${categoryNumber}&currentPage=${i}&sort=new<c:if test='${param.searchWord != null}'>&searchWord=${param.searchWord}</c:if>">${i}</a></li>
				                </c:otherwise>
				            </c:choose>
				        </c:forEach>
				    </ol>                
				    ...
				    <c:if test="${pvo.end < pvo.totalPages}">
				        <a href="?catno=${categoryNumber}&currentPage=${pvo.currentPage + 1}&sort=new<c:if test='${param.searchWord != null}'>&searchWord=${param.searchWord}</c:if>" class="next">next</a>
				    </c:if>                
				    
				    <a href="?catno=${categoryNumber}&currentPage=${pvo.totalPages}&sort=new<c:if test='${param.searchWord != null}'>&searchWord=${param.searchWord}</c:if>" class="last">last</a>
				</div>
